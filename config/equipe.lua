...
local function aviso(mensagem)
  ...
  return pandoc.RawBlock("html",
    '<div class="team-error">⚠ ' .. escapar(mensagem) .. "</div>")
end
    ...
    for _, grupo in ipairs(montar_grupos(cfg)) do
      ...
      if fotos ~= nil and #fotos > 0 then
        local function img(foto)
          return '<img class="mosaic-photo" src="'
        ...
        table.insert(html, '<div class="mosaic-group">')

        -- Rótulo e PRIMEIRA foto vão juntos dentro de .mosaic-lead, que é
        -- inline-flex e portanto indivisível. Sem isso, o rótulo poderia
        -- sobrar sozinho no fim de uma linha, longe do seu grupo.
        table.insert(html, '<span class="mosaic-lead">')
        if mostrar_titulos then
          table.insert(html,
            '<span class="mosaic-title">' .. escapar(grupo.titulo)
            .. ' <span class="mosaic-count">' .. #fotos .. "</span></span>")
        end
